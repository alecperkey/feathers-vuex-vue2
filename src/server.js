...
todoService.hooks({
  before: { // hook to add flare before saving
    all: softDelete()
  }
})
async function addBuiltInTodo (query) { // server-side built-in todo
  const list = await todoService.find({ query })
  if (list.length === 0) { todoService.create({ name: `${query.name}` }) }
}
